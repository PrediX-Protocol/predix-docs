...
### Quyền Hook <a href="#quyen-hook" id="quyen-hook"></a>
...
### Tính phí động <a href="#tinh-phi-dong" id="tinh-phi-dong"></a>
...
### Chống tấn công Sandwich <a href="#chong-tan-cong-sandwich" id="chong-tan-cong-sandwich"></a>

Hook theo dõi mỗi swap dưới dạng `(block.number, direction)` theo từng người dùng và pool. Nếu cùng một người dùng swap ngược chiều trong cùng một block, giao dịch sẽ bị hoàn lại với lỗi `SandwichDetected()`.**Router đáng tin cậy**: Hook trích xuất địa chỉ người dùng thực tế từ `hookData` thay vì sử dụng `msg.sender`, giúp Router có thể thực thi thay mặt người dùng mà không gây ra cảnh báo giả.
