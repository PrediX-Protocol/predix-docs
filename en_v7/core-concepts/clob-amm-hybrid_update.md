...
{% hint style="warning" %}
### You cannot trade the pool directly

Every swap on the YES-USDC pool must carry a **router-committed identity**. A swap that does not come through `PrediXRouter` — via UniversalRouter, the Uniswap web app, or `PoolManager` directly — **reverts** (`Hook_UntrustedCaller` / `Hook_MissingRouterCommit`). This is the anti-sandwich barrier by design, not a soft preference. Always route through `PrediXRouter`.
{% endhint %}
