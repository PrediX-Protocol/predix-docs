...
![4 oracle types: ChainlinkOracle (price, auto), ManualOracle (subjective, multisig 2/3), UMAOracle (decentralized, 48h dispute, Phase 2), Committee (t-of-N, Phase 3)](../_design/25-oracle-types.svg)

## ChainlinkOracle

Automated, permissionless.

![ChainlinkOracle: creator register(feed, threshold, snapshotAt) -> endTime passes -> anyone resolve(roundIdHint) -> verify adjacency + sequencer uptime -> outcome = price >= threshold](../_design/37-chainlink-oracle-flow.svg)
...
![Refund mode: oracle fails -> admin proposes enableRefundMode -> 48h timelock -> refundModeActive=true -> user burns min(YES,NO) pairs -> USDC pro-rata](../_design/39-refund-mode.svg)
