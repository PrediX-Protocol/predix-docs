...
![4 reward types: Streak (daily login, win streak) + Badge (NFT milestone) + Daily Challenge (3 random/day) + Reward Box (5% drop, VRF random) -> convert to PRX/USDC/NFT](../_design/57-rewards-overview.svg)
...
![Reward box: eligible trade -> 5% drop chance -> box sealed -> user opens -> Chainlink VRF random -> 80% PRX / 15% USDC / 5% rare NFT](../_design/20-reward-vrf.svg)
