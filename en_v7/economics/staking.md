...
![Staking: deposit PRX -> mint stkPRX (non-transferable) -> each week protocol distributes fee share USDC pro-rata -> claim USDC](../_design/07-staking-flow.svg)
...
![Lock boost: no lock 1.0x, 3mo 1.1x, 6mo 1.25x, 12mo 1.5x, 24mo 2.0x, 48mo max 2.5x yield + corresponding vePRX weight](../_design/59-lock-boost-tiers.svg)
...
![Unstake: Staked -> request unstake -> Cooldown 7 days (still earning yield) -> Withdrawable -> claim PRX to wallet](../_design/24-unstake-cooldown.svg)
