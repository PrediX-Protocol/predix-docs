...
| **Quote** | On-chain `quoteBuyYes()` (eth_call) | REST API | REST API |
...
![Router hybrid execution: User → Smart Router (stateless) → splits to CLOB (priority, 99-tick order book) + AMM (fallback, Uniswap v4 Hook, dynamic fees) → combined YES/NO tokens out. Single atomic tx.](assets/router-hybrid-execution.svg)
