...
![5 notification channels: push browser/mobile, email digest, Discord webhook, Telegram bot, in-app center](../_design/42-notification-channels.svg)
...
![Price alert: user set condition (YES > $0.70) → Indexer check every block → trigger → push + email + in-app](../_design/43-price-alert-flow.svg)
